            ...
            <div class="my-4 row flex justify-between">
                <form action="{{route('inspections.search_by_job_number')}}" method="POST">
                    ...
                    <div class="row flex items-center">
                        <input type="text" name="job_number" placeholder="Enter Job Number" class="appearance-none block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm placeholder-gray-400 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" required>
            ...
            @foreach($inspections as $inspection)
                <div class="mb-4 bg-white shadow overflow-hidden sm:rounded-lg">
                    <div class="px-4 py-5 sm:px-6">
                        <h3 class="row flex justify-between text-lg leading-6 font-medium text-gray-900 items-center">
                            <div class="row flex justify-start">
                                <div>EPS - {{$inspection->job_number}} - {{$inspection->site}} - {{$inspection->client->client}}</div>
                                @if(auth()->user()->client_id === null)
                                    <livewire:edit-site-name :site="$inspection->id" :client_id="$client_id" />
                                @endif
                            </div>
                            @if(auth()->user()->client_id == null)
                            ...
                            <div class="py-4 sm:py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                                @if(auth()->user()->client_id == null)
                                    <dd class="text-sm text-gray-900 sm:mt-0 sm:col-span-2">
                                        <div class="mt-4 row flex items-center">
                                            @isset($not_acceptable)
                                                @foreach($not_acceptable as $item)
                                                    @if($item['id'] == $inspection->id)
                                                        <div class="text-sm text-gray-500">Number of<span class="font-bold">&nbsp;Not Acceptable&nbsp;</span>checks: {{count($item['answers'])}}</div>
                                                        <livewire:not-acceptable-checks :answers="$item['answers']"/>
                                                    @endif
                                                @endforeach
                                            @endisset
                                        </div>
                            ...
                            <div class="row p-4 flex">
                                @if(auth()->user()->id == 1  || auth()->user()->id == 5 || auth()->user()->id == 8 || auth()->user()->id == 6)
                                    <form action="{{route('inspection.delete')}}" method="POST">
                                        ...
                                        <button class="ml-auto mr-4 py-1 text-xs md:text-sm px-2 flex ml-auto items-center justify-center btn-red" type="submit" onclick="return confirm('Are you sure you want to DELETE EPS - {{$inspection->job_number}} - {{$inspection->site}} - {{$inspection->client->client}}.  This can not be undone!!')">Delete</button>
                                    </form>
                                @endif
                                @if(auth()->user()->client_id == null)
