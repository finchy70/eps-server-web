<div>
    <button class="ml-4 py-1 text-xs px-1 btn-indigo" type="button" wire:click="show()">
        Show
    </button>
    @if($showNotAcceptable)
                    ...
                    <div>
                        ...
                        <div class="mt-3 sm:mt-5">
                            ...
                            @foreach($checks as $check)
                               <div class="mt-2 grid grid-cols-2 gap-4">
                                    <div>{{$check->question->section->name}}</div>
                                    <div>{{$check->question->question}}</div>
                               </div>
...
{{--                            Deactivate--}}
{{--                        </button>--}}
                        <button wire:click="$set('showNotAcceptable', false)" type="button" class="mt-3 w-full inline-flex justify-center rounded-md border border-gray-300 shadow-sm px-4 py-2 bg-indigo-100 hover:bg-indigo-300 text-base font-medium text-gray-700 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 sm:mt-0 sm:col-start-1 sm:text-sm">
                            Cancel
                        </button>
                    </div>
                </div>
