<div>
    <div>
        <div class="row flex justify-center">
            <div class="mt-4 mb-2 text-4xl text-indigo-600 font-bold">Question Setup - {{$section->name}}</div>
        </div>
        <div class="row flex justify-center">
            <div class="mt-2 mb-4 text-sm text-indigo-600 font-bold">Drag and Drop Questions into the desired order.</div>
        </div>
        <div class="mx-auto mb-4 max-w-2xl bg-white shadow overflow-hidden sm:rounded-md">
            <ul wire:sortable="updateQuestionOrder" class="divide-y divide-gray-200">
                    ...
                    <li wire:sortable.item="{{ $question->id }}" wire:key="question-{{ $question->id }}">
                        <div class="block hover:bg-gray-200">
                            <div class="row flex justify-between">
                                ...
                                <button onclick="return confirm('Are you sure you want to delete {{$question->question}}?') || event.stopImmediatePropagation()" wire:click="removeQuestion({{$question->id}})" class="my-4 mr-2 text-xs btn-red">Remove</button>
                            </div>
                        </div>
        ...
        <hr>
        <div class="row flex justify-center">
            <div class="mt-2 mb-4 text-sm text-indigo-600 font-bold">Add New Question</div>
        </div>
        <div class="p-4 mt-4 mx-auto max-w-2xl bg-white shadow overflow-hidden sm:rounded-md">
            <div>
                <form wire:submit.prevent="addQuestion">
                    <label for="email" class="block text-sm font-medium text-gray-700">New Question</label>
                    <div class="mt-1">
                        <input wire:model.lazy="question" type="text" required name="name" id="name" class="shadow-sm focus:ring-indigo-500 focus:border-indigo-500 block w-full sm:text-sm border-gray-300 rounded-md">
                    </div>
                    @error('question') <span class="text-xs text-red-600 italic">{{$message}}</span> @enderror
                    <div class="row flex justify-end">
                        <a href="{{route('setup.questions')}}" class="mr-4 mt-2 btn-yellow">Back</a>
                ...
                <div class="flex items-start">
                    <div class="ml-3 w-0 flex-1">
                        <p class="text-sm font-medium text-red-900">
                            You can't delete this question as it has answers stored against it!
                        </p>
                    </div>
